import{j as H,W as L}from"./index-DABIPWEa.js";import{W as V,S as j,O as G,P as _,a as O,V as h,M as K,C as N}from"./three-mUwG1-rm.js";import{r as p}from"./vendor-nf7bT_Uh.js";const X=`
...
}`;function Z({condition:x="partlycloudy",hourEq:y=12,intensity:g=1}){const w=p.useRef(null),n=p.useRef({condition:x,hourEq:y,intensity:g});return n.current={condition:x,hourEq:y,intensity:g},p.useEffect(()=>{const l=w.current;if(!l)return;let C=!1,W=0,a;try{a=new V({antialias:!1})}catch{return}a.setPixelRatio(Math.max(1,Math.min(window.devicePixelRatio||1,1.5)));const m=a.domElement;m.style.cssText="position:absolute;inset:0;display:block;width:100%;height:100%;",l.appendChild(m);const k=new j,A=new G(-1,1,1,-1,0,1),e={uRes:{value:new h(1,1)},uTime:{value:0},uElev:{value:.6},uSunPos:{value:new h(.5,.8)},uMoonPos:{value:new h(.5,-.4)},uCloud:{value:0},uDark:{value:0},uFog:{value:0},uRain:{value:0},uSnow:{value:0},uHail:{value:0},uWind:{value:0},uStars:{value:0},uFlash:{value:0},uBolt:{value:0},uBoltSeed:{value:0},uExc:{value:0},uIntensity:{value:1}},S=new _(2,2),b=new O({uniforms:e,vertexShader:"varying vec2 vUv; void main(){ vUv = uv; gl_Position = vec4(position.xy, 0.0, 1.0); }",fragmentShader:X});k.add(new K(S,b));const M=()=>{const o=l.clientWidth||1,i=l.clientHeight||1;a.setSize(o,i,!1),e.uRes.value.set(o*a.getPixelRatio(),i*a.getPixelRatio())},R=new ResizeObserver(M);R.observe(l),M();const T=["cloud","dark","fog","rain","snow","hail","wind","stars","exc"],r={};T.forEach(o=>{r[o]=0});let u=.2;const E=new N;let s=0,v=0,d=2;const U=1e3/30;let P=0;function B(){if(C||(W=requestAnimationFrame(B),document.hidden))return;const o=Math.min(E.getDelta(),.05),i=E.elapsedTime;e.uTime.value=i;const f=L[n.current.condition]||L.partlycloudy;let c=n.current.hourEq;f.forceNight&&(c=23.2),f.forceDay&&(c=Math.min(Math.max(c,8),17));const D=(c-6)/12*Math.PI,z=Math.sin(D);u+=(z-u)*Math.min(1,o*2.2),e.uElev.value=u;const F=.68+.26*Math.sin((c-6)/12*Math.PI-Math.PI/2)*-1;e.uSunPos.value.set(F,.18+u*.72),e.uMoonPos.value.set(1-F,.18-u*.72),T.forEach(t=>{r[t]+=((f[t]||0)-r[t])*Math.min(1,o*2.4),e["u"+t[0].toUpperCase()+t.slice(1)].value=r[t]}),e.uIntensity.value=n.current.intensity;const q=f.strike||0;if(q>0){d-=o*q,d<=0&&(d=1.6+Math.random()*4.5,s=1,v=Math.random()<.75?1:0,e.uBoltSeed.value=Math.random()*100);const t=.55+.45*Math.sin(i*90);e.uFlash.value=s*t*.85,e.uBolt.value=v*Math.pow(s,.4)*t,s=Math.max(0,s-o*3.4),s<.02&&(v=0)}else s=0,v=0,e.uFlash.value+=(0-e.uFlash.value)*Math.min(1,o*6),e.uBolt.value=0;const I=typeof performance<"u"?performance.now():Date.now();I-P>=U&&(P=I,a.render(k,A))}return B(),()=>{C=!0,cancelAnimationFrame(W),R.disconnect(),S.dispose(),b.dispose(),a.dispose();try{l.removeChild(m)}catch{}}},[]),H.jsx("div",{ref:w,"aria-hidden":"true",style:{position:"absolute",inset:0}})}export{L as WX_PRESETS,Z as default};
