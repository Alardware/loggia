import{r as h,j as H,W as I}from"./boot-CfYV6WJs.js";import{W as V,S as j,O as G,P as _,a as O,V as p,M as K,C as N}from"./three-CVm9K9Wx.js";import"./vendor-D3j9xVti.js";import"./index-DOwwyJxP.js";const X=`
...
}`;function $({condition:x="partlycloudy",hourEq:y=12,intensity:g=1}){const w=h.useRef(null),c=h.useRef({condition:x,hourEq:y,intensity:g});return c.current={condition:x,hourEq:y,intensity:g},h.useEffect(()=>{const l=w.current;if(!l)return;let b=!1,C=0,a;try{a=new V({antialias:!1})}catch{return}a.setPixelRatio(Math.max(1,Math.min(window.devicePixelRatio||1,1.5)));const d=a.domElement;d.style.cssText="position:absolute;inset:0;display:block;width:100%;height:100%;",l.appendChild(d);const W=new j,A=new G(-1,1,1,-1,0,1),e={uRes:{value:new p(1,1)},uTime:{value:0},uElev:{value:.6},uSunPos:{value:new p(.5,.8)},uMoonPos:{value:new p(.5,-.4)},uCloud:{value:0},uDark:{value:0},uFog:{value:0},uRain:{value:0},uSnow:{value:0},uHail:{value:0},uWind:{value:0},uStars:{value:0},uFlash:{value:0},uBolt:{value:0},uBoltSeed:{value:0},uExc:{value:0},uIntensity:{value:1}},k=new _(2,2),S=new O({uniforms:e,vertexShader:"varying vec2 vUv; void main(){ vUv = uv; gl_Position = vec4(position.xy, 0.0, 1.0); }",fragmentShader:X});W.add(new K(k,S));const M=()=>{const o=l.clientWidth||1,u=l.clientHeight||1;a.setSize(o,u,!1),e.uRes.value.set(o*a.getPixelRatio(),u*a.getPixelRatio())},R=new ResizeObserver(M);R.observe(l),M();const T=["cloud","dark","fog","rain","snow","hail","wind","stars","exc"],r={};T.forEach(o=>{r[o]=0});let i=.2;const E=new N;let s=0,v=0,m=2;const U=1e3/30;let P=0;function B(){if(b||(C=requestAnimationFrame(B),document.hidden||document.documentElement.classList.contains("loggia-ambient-on")))return;const o=Math.min(E.getDelta(),.05),u=E.elapsedTime;e.uTime.value=u;const f=I[c.current.condition]||I.partlycloudy;let n=c.current.hourEq;f.forceNight&&(n=23.2),f.forceDay&&(n=Math.min(Math.max(n,8),17));const D=(n-6)/12*Math.PI,z=Math.sin(D);i+=(z-i)*Math.min(1,o*2.2),e.uElev.value=i;const F=.68+.26*Math.sin((n-6)/12*Math.PI-Math.PI/2)*-1;e.uSunPos.value.set(F,.18+i*.72),e.uMoonPos.value.set(1-F,.18-i*.72),T.forEach(t=>{r[t]+=((f[t]||0)-r[t])*Math.min(1,o*2.4),e["u"+t[0].toUpperCase()+t.slice(1)].value=r[t]}),e.uIntensity.value=c.current.intensity;const q=f.strike||0;if(q>0){m-=o*q,m<=0&&(m=1.6+Math.random()*4.5,s=1,v=Math.random()<.75?1:0,e.uBoltSeed.value=Math.random()*100);const t=.55+.45*Math.sin(u*90);e.uFlash.value=s*t*.85,e.uBolt.value=v*Math.pow(s,.4)*t,s=Math.max(0,s-o*3.4),s<.02&&(v=0)}else s=0,v=0,e.uFlash.value+=(0-e.uFlash.value)*Math.min(1,o*6),e.uBolt.value=0;const L=typeof performance<"u"?performance.now():Date.now();L-P>=U&&(P=L,a.render(W,A))}return B(),()=>{b=!0,cancelAnimationFrame(C),R.disconnect(),k.dispose(),S.dispose(),a.dispose();try{l.removeChild(d)}catch{}}},[]),H.jsx("div",{ref:w,"aria-hidden":"true",style:{position:"absolute",inset:0}})}export{I as WX_PRESETS,$ as default};
