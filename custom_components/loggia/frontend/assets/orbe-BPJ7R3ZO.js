import{j as Ce,t as rt}from"./boot-BMdyy2wC.js";import{r as se}from"./vendor-nf7bT_Uh.js";import{W as nt,S as He,b as st,G as it,D as ot,R as lt,F as ut,N as ct,c as De,d as xe,e as R,V as be,a as ie,A as We,O as pt,M as dt,P as ht,f as vt,g as ze,h as Ee,L as Ge,H as mt,B as Ye,i as x,j as Xe}from"./three-BC6Dzy3a.js";import"./index-CQ60MUOh.js";function ft(oe){let B=!0;class le{constructor(a,r){this.camera=a,this.dom=r,this.target=new R,this.enablePan=!1,this.enableDamping=!0,this.dampingFactor=.06,this.minDistance=1,this.maxDistance=100,this.rotateSpeed=.5,this.zoomSpeed=.7,this.autoRotate=!1,this.autoRotateSpeed=.3;const p=a.position.clone().sub(this.target);this.r=p.length()||5,this.theta=Math.atan2(p.x,p.z),this.phi=Math.acos(Math.max(-1,Math.min(1,p.y/this.r))),this.dTheta=0,this.dPhi=0,this.dR=0,this._drag=!1,this._x=0,this._y=0,r.style.touchAction="none",r.addEventListener("pointerdown",s=>{this._drag=!0,this._x=s.clientX,this._y=s.clientY;try{r.setPointerCapture(s.pointerId)}catch{}}),r.addEventListener("pointerup",()=>{this._drag=!1}),r.addEventListener("pointerleave",()=>{this._drag=!1}),r.addEventListener("pointermove",s=>{if(!this._drag)return;const u=r.clientWidth||1;this.dTheta-=2*Math.PI*(s.clientX-this._x)/u*this.rotateSpeed,this.dPhi-=2*Math.PI*(s.clientY-this._y)/u*this.rotateSpeed,this._x=s.clientX,this._y=s.clientY}),r.addEventListener("wheel",s=>{s.preventDefault(),this.dR+=Math.sign(s.deltaY)*.28*this.zoomSpeed},{passive:!1})}update(){this.autoRotate&&(this.theta-=this.autoRotateSpeed*.012),this.theta+=this.dTheta,this.phi+=this.dPhi,this.r+=this.dR;const a=this.enableDamping?Math.max(0,1-this.dampingFactor*3):0;this.dTheta*=a,this.dPhi*=a,this.dR*=a,this.phi=Math.max(.08,Math.min(Math.PI-.08,this.phi)),this.r=Math.max(this.minDistance,Math.min(this.maxDistance,this.r));const r=Math.sin(this.phi);this.camera.position.set(this.target.x+this.r*r*Math.sin(this.theta),this.target.y+this.r*Math.cos(this.phi),this.target.z+this.r*r*Math.cos(this.theta)),this.camera.lookAt(this.target)}}const K={repos:{label:"REPOS",flow:.35,turb:.35,energy:.3,spin:.1},flux:{label:"FLUX",flow:1,turb:.65,energy:.6,spin:.22},analyse:{label:"ANALYSE",flow:1.7,turb:.95,energy:.82,spin:.55},turbu:{label:"TURBULENCE",flow:2,turb:1.5,energy:.9,spin:.4},surch:{label:"SURCHARGE",flow:2.8,turb:1.8,energy:1,spin:.85},ecoute:{label:"ÉCOUTE",flow:.85,turb:.42,energy:.55,spin:.14},pense:{label:"RÉFLEXION",flow:2.1,turb:.8,energy:.86,spin:.72},parle:{label:"RÉPONSE",flow:.75,turb:.34,energy:.42,spin:.16}},q=[{deep:[.02,.16,.62],mid:[.22,.66,1],hot:[.92,.99,1]},{deep:[.2,.06,.55],mid:[.62,.42,1],hot:[.99,.96,1]},{deep:[.52,.14,.01],mid:[1,.58,.12],hot:[1,.94,.76]}],e={mode:"repos",level:1,charge:0,energy:.3,flow:.35,turb:.35,pulse:0,micLevel:0,pal:0,count:0,curves:0,phase:0,iph:0,dir:1,force:null},f=[],g=oe,o=new nt({antialias:!1,alpha:!0,powerPreference:"high-performance",premultipliedAlpha:!1}),V=Math.min(1.5,window.devicePixelRatio||1);o.setPixelRatio(V),o.setClearColor(0,0),o.autoClear=!1,o.domElement.style.background="transparent",o.domElement.style.display="block",g.appendChild(o.domElement);const Fe=new He,H=new st(36,1,.1,60);H.position.set(0,.25,4.3);const y=new le(H,o.domElement);y.enablePan=!1,y.enableDamping=!0,y.dampingFactor=.06,y.minDistance=2.2,y.maxDistance=9,y.rotateSpeed=.5,y.zoomSpeed=.7,y.autoRotate=!0,y.autoRotateSpeed=.3;const C=new it;Fe.add(C);const W=256,ue=64,Q=new Float32Array(W*ue*4),G=new ot(Q,W,ue,lt,ut);G.magFilter=G.minFilter=ct,G.generateMipmaps=!1;const l={uTime:{value:0},uFlow:{value:.35},uTurb:{value:.35},uEnergy:{value:.3},uPulse:{value:0},uMic:{value:0},uPhase:{value:0},uIph:{value:0},uDirS:{value:1},uCurves:{value:G},uCurveTex:{value:new be(W,ue)},uPointer:{value:new R(0,0,9)},uPointerOn:{value:0},uWaves:{value:[new xe(-9,0,0,0),new xe(-9,0,0,0),new xe(-9,0,0,0),new xe(-9,0,0,0)]},uDeep:{value:new De},uMid:{value:new De},uHot:{value:new De},uVoice:{value:0},uPitch:{value:0},uPx:{value:1}},ce=`
  ...
  vec3 heat(float h, float depth){
    ...
    return c * (0.45 + 0.55*depth);
      ...
      }`});let ke=1,Y=ke;const n=()=>{Y=Y+1831565813|0;let t=Math.imul(Y^Y>>>15,1|Y);return t=t+Math.imul(t^t>>>7,61|t)^t,((t^t>>>14)>>>0)/4294967296},D=()=>{const t=n()*2-1,a=n()*6.2831,r=Math.sqrt(Math.max(0,1-t*t));return new R(Math.cos(a)*r,t,Math.sin(a)*r)},we=(t,a,r)=>Math.sin(t*3.1)*Math.cos(a*2.7)+Math.sin(a*3.9)*Math.cos(r*3.1)+Math.sin(r*4.3)*Math.cos(t*2.3);let z=null,E=null;function Je(t,a){const r=a.clone().applyAxisAngle(D(),(n()-.5)*1).normalize();let p=D();p.addScaledVector(r,-p.dot(r)).normalize();const s=3+n()*3.6,u=n()*10,b=.96+n()*.1,L=n()<.45,T=L?.24+Math.pow(n(),1.6)*.3:.84+n()*.14,X=.3+n()*.4,O=Math.log(.5)/Math.log(X),k=L?2.4+n()*2.2:1.3+n()*1,i=new R,v=new R;for(let I=0;I<W;I++){const S=I/(W-1),ve=s*S;i.set(r.x+we(S*2.1+u,u,0)*.13,r.y+we(0,S*2.4+u,u)*.13,r.z+we(u,0,S*1.7+u)*.13).normalize(),v.copy(p).applyAxisAngle(i,ve);const me=Math.pow(Math.sin(3.14159265*Math.pow(S,O)),k),m=b-(b-T)*me+.045*Math.sin(S*4.1+u)+.022*Math.sin(S*9.3-u*2);v.multiplyScalar(Math.max(.05,m));const d=(t*W+I)*4;Q[d]=v.x,Q[d+1]=v.y,Q[d+2]=v.z,Q[d+3]=1}return s}function ye(t){Y=ke+t*7919|0,z&&(C.remove(z),z.geometry.dispose(),z=null),E&&(C.remove(E),E.geometry.dispose(),E=null);const a=Math.min(ue,13+t*3),r=[D(),D(),D(),D(),D()],p=[];for(let c=0;c<a;c++)p.push(Je(c,r[Math.floor(n()*r.length)]));G.needsUpdate=!0;const s=24e4+t*55e3,u=p.reduce((c,j)=>c+j,0),b=new Float32Array(s*3),L=new Float32Array(s),T=new Float32Array(s),X=new Float32Array(s),O=new Float32Array(s),k=new Float32Array(s),i=new Float32Array(s),v=new Float32Array(s),I=new Float32Array(s),S=new Float32Array(s),ve=new Float32Array(s);let me=0,m=0;for(let c=0;c<a&&m<s;c++){const j=Math.floor(s*p[c]/u),ae=.04+Math.pow(n(),1.6)*.09,re=.45+n()*.8;for(let Ve=0;Ve<j&&m<s;Ve++,m++){L[m]=c,T[m]=n(),X[m]=n(),O[m]=n()*6.2831,k[m]=ae*Math.pow(n(),2)*(1+(n()<.07?2.6:0)),i[m]=re*(.4+n()*.9),I[m]=n()<.07?.45+n()*.75:0;const Re=n();let N,J,ne;Re<.01?(N=2.6+n()*1.8,J=1.9+n()*.9,ne=1.9+n()*1.4):Re<.055?(N=1.6+n()*1,J=1.25+n()*.6,ne=.9+n()*.6):Re<.3?(N=.95+n()*.6,J=.72+n()*.38,ne=.65+n()*.5):(N=.58+n()*.47,J=.34+n()*.28,ne=.45+n()*.5),v[m]=N,S[m]=J,ve[m]=ne,me+=J*N*N}}const d=m,at=1.98*d/Math.max(me,1e-6);for(let c=0;c<d;c++)S[c]*=at;const w=new Ye;w.setAttribute("position",new x(b.subarray(0,d*3),3)),w.setAttribute("aCurve",new x(L.subarray(0,d),1)),w.setAttribute("aU",new x(T.subarray(0,d),1)),w.setAttribute("aSeed",new x(X.subarray(0,d),1)),w.setAttribute("aAng",new x(O.subarray(0,d),1)),w.setAttribute("aRad",new x(k.subarray(0,d),1)),w.setAttribute("aFray",new x(i.subarray(0,d),1)),w.setAttribute("aSize",new x(v.subarray(0,d),1)),w.setAttribute("aSpray",new x(I.subarray(0,d),1)),w.setAttribute("aLum",new x(S.subarray(0,d),1)),w.setAttribute("aSpd",new x(ve.subarray(0,d),1)),w.boundingSphere=new ze(new R,4),z=new Xe(w,Te),C.add(z);const fe=7200+t*1700,ge=new Float32Array(fe*3),je=new Float32Array(fe),Ne=new Float32Array(fe);for(let c=0;c<fe;c++){const j=D(),ae=n(),re=ae<.34?.05+Math.pow(n(),1.3)*.44:ae<.62?.25+Math.pow(n(),.6)*.8:ae<.94?.92+n()*.2:1.06+Math.pow(n(),2.6)*.34;ge[c*3]=j.x*re,ge[c*3+1]=j.y*re,ge[c*3+2]=j.z*re,je[c]=n()<.035?1.5+n()*1.4:.45+n()*.7,Ne[c]=n()}const te=new Ye;te.setAttribute("position",new x(ge,3)),te.setAttribute("aSize",new x(je,1)),te.setAttribute("aSeed",new x(Ne,1)),te.boundingSphere=new ze(new R,4),E=new Xe(te,Oe),C.add(E),e.count=d,e.curves=a}const Me={type:mt,minFilter:Ge,magFilter:Ge,depthBuffer:!1};let A=null,F=[];const Z=new He,pe=new pt(-1,1,1,-1,0,1),de=new dt(new ht(2,2));Z.add(de);const Se="varying vec2 vUv; void main(){ vUv = uv; gl_Position = vec4(position.xy, 0.0, 1.0); }",Ae=new ie({uniforms:{tex:{value:null}},vertexShader:Se,fragmentShader:"uniform sampler2D tex; varying vec2 vUv; void main(){ gl_FragColor = texture2D(tex, vUv); }"}),U=new ie({uniforms:{tex:{value:null},dir:{value:new be(1,0)},texel:{value:new be}},vertexShader:Se,fragmentShader:`uniform sampler2D tex; uniform vec2 dir, texel; varying vec2 vUv;
    ...
    }`});function $(){const t=g.clientWidth,a=g.clientHeight;if(!t||!a)return;o.setSize(t,a),H.aspect=t/a,H.updateProjectionMatrix(),l.uPx.value=a*V*.0105;const r=Math.round(t*V),p=Math.round(a*V);A&&A.dispose(),F.forEach(s=>{s.a.dispose(),s.b.dispose()}),A=new Ee(r,p,Me),F=[2,4,8,16].map(s=>{const u=Math.max(2,Math.round(r/s)),b=Math.max(2,Math.round(p/s));return{a:new Ee(u,b,Me),b:new Ee(u,b,Me),w:u,h:b}})}window.addEventListener("resize",$);const he=window.ResizeObserver?new ResizeObserver(()=>$()):null;he&&he.observe(g);function Ke(){o.setRenderTarget(A),o.clear(),o.render(Fe,H);let t=A.texture;for(const a of F)de.material=Ae,Ae.uniforms.tex.value=t,o.setRenderTarget(a.a),o.render(Z,pe),de.material=U,U.uniforms.texel.value.set(1/a.w,1/a.h),U.uniforms.dir.value.set(1,0),U.uniforms.tex.value=a.a.texture,o.setRenderTarget(a.b),o.render(Z,pe),U.uniforms.dir.value.set(0,1),U.uniforms.tex.value=a.b.texture,o.setRenderTarget(a.a),o.render(Z,pe),t=a.a.texture;de.material=P,P.uniforms.tScene.value=A.texture,P.uniforms.tB0.value=F[0].a.texture,P.uniforms.tB1.value=F[1].a.texture,P.uniforms.tB2.value=F[2].a.texture,P.uniforms.tB3.value=F[3].a.texture,o.setRenderTarget(null),o.render(Z,pe)}const Be=new vt,qe=new be,Qe=new ze(new R,1),Pe=new R,M=o.domElement;let ee=null,Ue=0;M.addEventListener("pointerdown",t=>{ee={x:t.clientX,y:t.clientY},M.style.cursor="grabbing"}),M.addEventListener("pointerup",t=>{ee&&Math.hypot(t.clientX-ee.x,t.clientY-ee.y)<6&&Ze(),ee=null,M.style.cursor="grab"}),M.addEventListener("pointermove",t=>{const a=M.getBoundingClientRect();qe.set((t.clientX-a.left)/a.width*2-1,-((t.clientY-a.top)/a.height)*2+1),Be.setFromCamera(qe,H),Be.ray.intersectSphere(Qe,Pe)?(C.worldToLocal(Pe),l.uPointer.value.copy(Pe),l.uPointerOn.value=1,Ue=performance.now()):l.uPointerOn.value=0}),M.addEventListener("pointerleave",()=>{l.uPointerOn.value=0});function Ze(){f.push({r:0,life:1}),e.pulse=Math.min(1.7,e.pulse+1),e.charge+=.2,e.charge>=1&&e.level<6?(e.charge=0,e.level++,ye(e.level),f.push({r:0,life:1.4}),e.pulse=1.7):e.level>=6&&(e.charge=1)}function $e(t){e.mode=t,e.pulse=Math.min(1.7,e.pulse+.55),f.push({r:0,life:.85})}const et={last:performance.now(),dt(){const t=performance.now(),a=(t-this.last)/1e3;return this.last=t,a}};let _=0;const Le=(t,a,r)=>t+(a-t)*r,h={deep:[0,0,0],mid:[0,0,0],hot:[0,0,0]};h.deep=q[0].deep.slice(),h.mid=q[0].mid.slice(),h.hot=q[0].hot.slice();function _e(){if(!B)return;requestAnimationFrame(_e);const t=Math.round(g.clientWidth*V),a=Math.round(g.clientHeight*V);if((M.width!==t||M.height!==a)&&$(),!A)return;const r=Math.min(.05,et.dt());_+=r;const p=K[e.mode];e.micLevel*=.9;let s;e.force!==null?s=e.force:e.mode==="parle"?s=Math.max(0,.34+.3*Math.sin(_*7.3)+.22*Math.sin(_*11.9+1.7)+.14*Math.sin(_*19.1)):s=0;const u=e.force!==null||e.mode==="parle",b=Math.max(e.micLevel,Math.min(1,s));l.uVoice.value=s,l.uPitch.value=0;const L=Math.min(1,r*4),T=Math.min(1,r*13);e.energy+=(p.energy+b*.55+e.pulse*.25-e.energy)*(u?T:L),e.flow+=(p.flow+b*1.9+e.pulse*.8-e.flow)*(u?T:L),e.turb+=(p.turb+b*.75+e.pulse*.45-e.turb)*(u?T:L),u&&(y.autoRotateSpeed=.2+s*.9),e.pulse*=Math.pow(.15,r),e.charge=Math.max(0,e.charge-r*.02),(e.mode==="analyse"||e.mode==="surch"||e.mode==="pense")&&(e.charge=Math.min(1,e.charge+r*.035)),e.charge>=1&&e.level<6&&(e.charge=0,e.level++,ye(e.level),f.push({r:0,life:1.3})),performance.now()-Ue>450&&(l.uPointerOn.value*=Math.pow(.05,r)),e.phase+=r*e.flow;const X=e.mode==="ecoute"||e.micLevel>.03?-1:1;e.dir+=(X-e.dir)*Math.min(1,r*2.2),e.iph+=r*(.22+.15*e.flow)*e.dir,l.uPhase.value=e.phase,l.uIph.value=e.iph,l.uDirS.value=e.dir>=0?1:-1;const O=q[e.pal],k=Math.min(1,r*3);for(let i=0;i<3;i++)h.deep[i]=Le(h.deep[i],O.deep[i],k),h.mid[i]=Le(h.mid[i],O.mid[i],k),h.hot[i]=Le(h.hot[i],O.hot[i],k);l.uDeep.value.setRGB(h.deep[0],h.deep[1],h.deep[2]),l.uMid.value.setRGB(h.mid[0],h.mid[1],h.mid[2]),l.uHot.value.setRGB(h.hot[0],h.hot[1],h.hot[2]),P.uniforms.uExp.value=1.34-e.energy*.16,P.uniforms.uBloom.value=.9-e.energy*.34,l.uTime.value=_,l.uEnergy.value=e.energy,l.uFlow.value=e.flow,l.uTurb.value=e.turb,l.uPulse.value=e.pulse,l.uMic.value=b;for(let i=f.length-1;i>=0;i--){const v=f[i];v.r+=1.9*r,v.life-=r*.8,(v.life<=0||v.r>2.6)&&f.splice(i,1)}for(let i=0;i<4;i++){const v=f[i];l.uWaves.value[i].set(v?v.r:-9,v?Math.max(0,v.life):0,0,0)}y.autoRotateSpeed=.2+p.spin*1.2,C.rotation.x=Math.sin(_*.1)*.1,C.rotation.z=Math.cos(_*.07)*.07,y.update(),Ke()}$(),ye(1),_e();const Ie={idle:"repos",listening:"ecoute",thinking:"pense",speaking:"parle"},tt=Object.fromEntries(Object.entries(Ie).map(([t,a])=>[a,t]));return{setState(t){const a=Ie[t]||(K[t]?t:null);a&&a!==e.mode&&$e(a)},setLevel(t){e.force=Math.max(0,Math.min(1,+t||0))},releaseLevel(){e.force=null},pulse(){f.push({r:0,life:1}),e.pulse=Math.min(1.7,e.pulse+1)},get state(){return tt[e.mode]||e.mode},dispose(){if(B){B=!1,window.removeEventListener("resize",$),he&&he.disconnect();try{z&&z.geometry.dispose(),E&&E.geometry.dispose(),Te.dispose(),Oe.dispose(),Ae.dispose(),U.dispose(),P.dispose(),G.dispose(),A&&A.dispose(),F.forEach(t=>{t.a.dispose(),t.b.dispose()}),o.dispose(),M.parentNode&&M.parentNode.removeChild(M)}catch{}}}}}function yt({etat:oe="idle",niveau:B=null,onToucher:le=null,taille:K=240}){const q={width:K,height:K,margin:"0 auto",display:"block",position:"relative"},e=se.useRef(null),f=se.useRef(null);return se.useEffect(()=>{if(!e.current)return;let g=null;try{g=ft(e.current)}catch(o){console.warn("Loggia : orbe indisponible",o);return}return f.current=g,()=>{f.current=null,g.dispose()}},[]),se.useEffect(()=>{f.current&&f.current.setState(oe)},[oe]),se.useEffect(()=>{const g=f.current;g&&(B==null?g.releaseLevel():g.setLevel(B))},[B]),le?Ce.jsx("button",{type:"button",onClick:le,"aria-label":rt("Parler à l’assistant"),style:{...q,border:"none",padding:0,background:"transparent",cursor:"pointer"},children:Ce.jsx("span",{ref:e,style:{display:"block",width:"100%",height:"100%"}})}):Ce.jsx("div",{ref:e,style:q})}export{ft as creerOrbe,yt as default};
