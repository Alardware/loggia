...
}`;function I({drift:r=!0,exposure:i=1.38,limitMag:l=8.8}){const n=v.useRef(null),s=v.useRef({drift:r,exposure:i});return s.current={drift:r,exposure:i},v.useEffect(()=>{const t=n.current;if(!t)return;let f=!1,p=0,a;try{a=new k({antialias:!1})}catch{return}a.setPixelRatio(Math.max(1,Math.min(window.devicePixelRatio||1,1.5)));const u=a.domElement;u.style.cssText="position:absolute;inset:0;display:block;width:100%;height:100%;",t.appendChild(u);const d=new A,C=new P(-1,1,1,-1,0,1),M=S.slice(0,20).map(e=>new E(e.x,e.y,e.m,e.sp)),q=S.slice(0,20).map(e=>new T(...e.c)),o={cx:.6,cy:1,span:30},c={uRes:{value:new R(1,1)},uTime:{value:0},uCenter:{value:new R(o.cx,o.cy)},uSpan:{value:o.span},uExposure:{value:i},uNebula:{value:1},uDust:{value:.75},uLimitMag:{value:l},uLoop:{value:0},uStar:{value:M},uStarCol:{value:q}},m=new L(2,2),h=new F({uniforms:c,vertexShader:"varying vec2 vUv; void main(){ vUv = uv; gl_Position = vec4(position.xy, 0.0, 1.0); }",fragmentShader:U});d.add(new z(m,h));const x=()=>{const e=t.clientWidth||1,y=t.clientHeight||1;a.setSize(e,y,!1),c.uRes.value.set(e*a.getPixelRatio(),y*a.getPixelRatio())},b=new ResizeObserver(x);b.observe(t),x();const g=new N;function w(){if(f||(p=requestAnimationFrame(w),document.hidden))return;const e=g.elapsedTime;g.getDelta(),c.uTime.value=e,c.uExposure.value=s.current.exposure,s.current.drift?c.uCenter.value.set(o.cx+Math.sin(e*.012)*1.6,o.cy+Math.cos(e*.009)*.8):c.uCenter.value.set(o.cx,o.cy),a.render(d,C)}return w(),()=>{f=!0,cancelAnimationFrame(p),b.disconnect(),m.dispose(),h.dispose(),a.dispose();try{t.removeChild(u)}catch{}}},[]),O.jsx("div",{ref:n,"aria-hidden":"true",style:{position:"absolute",inset:0}})}export{I as default};
