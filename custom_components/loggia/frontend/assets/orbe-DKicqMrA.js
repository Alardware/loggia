import{r as K,j as Ce,t as st}from"./boot-D-LdtJkh.js";import{W as nt,S as We,b as it,G as ot,D as lt,R as ut,F as ct,N as pt,c as ze,d as be,e as L,V as we,a as ie,A as Ge,O as dt,M as ht,P as vt,f as mt,g as De,h as Te,L as Ye,H as ft,B as Xe,i as x,j as Je}from"./three-CVm9K9Wx.js";import"./vendor-D3j9xVti.js";import"./index-BuOQbS7u.js";const gt=(()=>{try{return window.matchMedia("(prefers-reduced-motion: reduce)").matches}catch{return!1}})();function xt(oe){let B=!0;class le{constructor(a,r){this.camera=a,this.dom=r,this.target=new L,this.enablePan=!1,this.enableDamping=!0,this.dampingFactor=.06,this.minDistance=1,this.maxDistance=100,this.rotateSpeed=.5,this.zoomSpeed=.7,this.autoRotate=!1,this.autoRotateSpeed=.3;const u=a.position.clone().sub(this.target);this.r=u.length()||5,this.theta=Math.atan2(u.x,u.z),this.phi=Math.acos(Math.max(-1,Math.min(1,u.y/this.r))),this.dTheta=0,this.dPhi=0,this.dR=0,this._drag=!1,this._x=0,this._y=0,r.style.touchAction="none",r.addEventListener("pointerdown",n=>{this._drag=!0,this._x=n.clientX,this._y=n.clientY;try{r.setPointerCapture(n.pointerId)}catch{}}),r.addEventListener("pointerup",()=>{this._drag=!1}),r.addEventListener("pointerleave",()=>{this._drag=!1}),r.addEventListener("pointermove",n=>{if(!this._drag)return;const c=r.clientWidth||1;this.dTheta-=2*Math.PI*(n.clientX-this._x)/c*this.rotateSpeed,this.dPhi-=2*Math.PI*(n.clientY-this._y)/c*this.rotateSpeed,this._x=n.clientX,this._y=n.clientY}),r.addEventListener("wheel",n=>{n.preventDefault(),this.dR+=Math.sign(n.deltaY)*.28*this.zoomSpeed},{passive:!1})}update(a=1/60){this.autoRotate&&(this.theta-=this.autoRotateSpeed*.72*a),this.theta+=this.dTheta,this.phi+=this.dPhi,this.r+=this.dR;const r=this.enableDamping?Math.max(0,1-this.dampingFactor*3):0;this.dTheta*=r,this.dPhi*=r,this.dR*=r,this.phi=Math.max(.08,Math.min(Math.PI-.08,this.phi)),this.r=Math.max(this.minDistance,Math.min(this.maxDistance,this.r));const u=Math.sin(this.phi);this.camera.position.set(this.target.x+this.r*u*Math.sin(this.theta),this.target.y+this.r*Math.cos(this.phi),this.target.z+this.r*u*Math.cos(this.theta)),this.camera.lookAt(this.target)}}const Q={repos:{label:"REPOS",flow:.32,turb:.3,energy:.28,spin:.1},flux:{label:"FLUX",flow:1,turb:.65,energy:.6,spin:.22},analyse:{label:"ANALYSE",flow:1.7,turb:.95,energy:.82,spin:.55},turbu:{label:"TURBULENCE",flow:2,turb:1.5,energy:.9,spin:.4},surch:{label:"SURCHARGE",flow:2.8,turb:1.8,energy:1,spin:.85},ecoute:{label:"ÉCOUTE",flow:.85,turb:.42,energy:.55,spin:.14},pense:{label:"RÉFLEXION",flow:2.1,turb:.8,energy:.86,spin:.72},parle:{label:"RÉPONSE",flow:.75,turb:.34,energy:.42,spin:.16}},U=[{deep:[.02,.16,.62],mid:[.22,.66,1],hot:[.92,.99,1]},{deep:[.2,.06,.55],mid:[.62,.42,1],hot:[.99,.96,1]},{deep:[.52,.14,.01],mid:[1,.58,.12],hot:[1,.94,.76]}],ue={parle:{deep:[.02,.28,.4],mid:[.31,.85,.92],hot:[.86,.99,1]},chaud:{deep:[.55,.22,.02],mid:[.98,.62,.38],hot:[1,.94,.82]},froid:{deep:[0,.3,.5],mid:[.42,.86,1],hot:[.94,1,1]},bien:{deep:[.02,.36,.05],mid:[.2,.9,.3],hot:[.7,1,.62]},alerte:{deep:[.55,.02,.02],mid:[1,.16,.12],hot:[1,.5,.42]}},e={mode:"repos",level:1,charge:0,energy:.3,flow:.35,turb:.35,pulse:0,micLevel:0,pal:0,count:0,curves:0,phase:0,iph:0,dir:1,force:null,teinte:null},f=[],g=oe,i=new nt({antialias:!1,alpha:!0,powerPreference:"high-performance"}),R=Math.min(1.5,window.devicePixelRatio||1);i.setPixelRatio(R),i.setClearColor(0,0),i.autoClear=!1,i.domElement.style.background="transparent",i.domElement.style.display="block",g.appendChild(i.domElement);const Fe=new We,H=new it(36,1,.1,60);H.position.set(0,.25,4.2);const y=new le(H,i.domElement);y.enablePan=!1,y.enableDamping=!0,y.dampingFactor=.06,y.minDistance=2.2,y.maxDistance=9,y.rotateSpeed=.5,y.zoomSpeed=.7,y.autoRotate=!0,y.autoRotateSpeed=.3;const C=new ot;Fe.add(C);const W=256,ce=64,Z=new Float32Array(W*ce*4),G=new lt(Z,W,ce,ut,ct);G.magFilter=G.minFilter=pt,G.generateMipmaps=!1;const l={uTime:{value:0},uFlow:{value:.35},uTurb:{value:.35},uEnergy:{value:.3},uPulse:{value:0},uMic:{value:0},uPhase:{value:0},uIph:{value:0},uDirS:{value:1},uCurves:{value:G},uCurveTex:{value:new we(W,ce)},uPointer:{value:new L(0,0,9)},uPointerOn:{value:0},uWaves:{value:[new be(-9,0,0,0),new be(-9,0,0,0),new be(-9,0,0,0),new be(-9,0,0,0)]},uDeep:{value:new ze},uMid:{value:new ze},uHot:{value:new ze},uVoice:{value:0},uPitch:{value:0},uPx:{value:1}},pe=`
  ...
  vec3 heat(float h, float depth){
    ...
    return c * (0.45 + 0.55*depth);
      ...
      }`});let ke=1,Y=ke;const s=()=>{Y=Y+1831565813|0;let t=Math.imul(Y^Y>>>15,1|Y);return t=t+Math.imul(t^t>>>7,61|t)^t,((t^t>>>14)>>>0)/4294967296},z=()=>{const t=s()*2-1,a=s()*6.2831,r=Math.sqrt(Math.max(0,1-t*t));return new L(Math.cos(a)*r,t,Math.sin(a)*r)},ye=(t,a,r)=>Math.sin(t*3.1)*Math.cos(a*2.7)+Math.sin(a*3.9)*Math.cos(r*3.1)+Math.sin(r*4.3)*Math.cos(t*2.3);let D=null,T=null;function Ke(t,a){const r=a.clone().applyAxisAngle(z(),(s()-.5)*1).normalize();let u=z();u.addScaledVector(r,-u.dot(r)).normalize();const n=3+s()*3.6,c=s()*10,b=.96+s()*.1,E=s()<.45,O=E?.24+Math.pow(s(),1.6)*.3:.84+s()*.14,X=.3+s()*.4,q=Math.log(.5)/Math.log(X),k=E?2.4+s()*2.2:1.3+s()*1,o=new L,v=new L;for(let N=0;N<W;N++){const S=N/(W-1),me=n*S;o.set(r.x+ye(S*2.1+c,c,0)*.13,r.y+ye(0,S*2.4+c,c)*.13,r.z+ye(c,0,S*1.7+c)*.13).normalize(),v.copy(u).applyAxisAngle(o,me);const fe=Math.pow(Math.sin(3.14159265*Math.pow(S,q)),k),m=b-(b-O)*fe+.045*Math.sin(S*4.1+c)+.022*Math.sin(S*9.3-c*2);v.multiplyScalar(Math.max(.05,m));const d=(t*W+N)*4;Z[d]=v.x,Z[d+1]=v.y,Z[d+2]=v.z,Z[d+3]=1}return n}function Me(t){Y=ke+t*7919|0,D&&(C.remove(D),D.geometry.dispose(),D=null),T&&(C.remove(T),T.geometry.dispose(),T=null);const a=Math.min(ce,13+t*3),r=[z(),z(),z(),z(),z()],u=[];for(let p=0;p<a;p++)u.push(Ke(p,r[Math.floor(s()*r.length)]));G.needsUpdate=!0;const n=24e4+t*55e3,c=u.reduce((p,j)=>p+j,0),b=new Float32Array(n*3),E=new Float32Array(n),O=new Float32Array(n),X=new Float32Array(n),q=new Float32Array(n),k=new Float32Array(n),o=new Float32Array(n),v=new Float32Array(n),N=new Float32Array(n),S=new Float32Array(n),me=new Float32Array(n);let fe=0,m=0;for(let p=0;p<a&&m<n;p++){const j=Math.floor(n*u[p]/c),re=.04+Math.pow(s(),1.6)*.09,se=.45+s()*.8;for(let He=0;He<j&&m<n;He++,m++){E[m]=p,O[m]=s(),X[m]=s(),q[m]=s()*6.2831,k[m]=re*Math.pow(s(),2)*(1+(s()<.07?2.6:0)),o[m]=se*(.4+s()*.9),N[m]=s()<.07?.45+s()*.75:0;const Re=s();let V,J,ne;Re<.01?(V=2.6+s()*1.8,J=1.9+s()*.9,ne=1.9+s()*1.4):Re<.055?(V=1.6+s()*1,J=1.25+s()*.6,ne=.9+s()*.6):Re<.3?(V=.95+s()*.6,J=.72+s()*.38,ne=.65+s()*.5):(V=.58+s()*.47,J=.34+s()*.28,ne=.45+s()*.5),v[m]=V,S[m]=J,me[m]=ne,fe+=J*V*V}}const d=m,rt=1.98*d/Math.max(fe,1e-6);for(let p=0;p<d;p++)S[p]*=rt;const w=new Xe;w.setAttribute("position",new x(b.subarray(0,d*3),3)),w.setAttribute("aCurve",new x(E.subarray(0,d),1)),w.setAttribute("aU",new x(O.subarray(0,d),1)),w.setAttribute("aSeed",new x(X.subarray(0,d),1)),w.setAttribute("aAng",new x(q.subarray(0,d),1)),w.setAttribute("aRad",new x(k.subarray(0,d),1)),w.setAttribute("aFray",new x(o.subarray(0,d),1)),w.setAttribute("aSize",new x(v.subarray(0,d),1)),w.setAttribute("aSpray",new x(N.subarray(0,d),1)),w.setAttribute("aLum",new x(S.subarray(0,d),1)),w.setAttribute("aSpd",new x(me.subarray(0,d),1)),w.boundingSphere=new De(new L,4),D=new Je(w,Oe),C.add(D);const ge=7200+t*1700,xe=new Float32Array(ge*3),je=new Float32Array(ge),Ve=new Float32Array(ge);for(let p=0;p<ge;p++){const j=z(),re=s(),se=re<.34?.05+Math.pow(s(),1.3)*.44:re<.62?.25+Math.pow(s(),.6)*.8:re<.94?.92+s()*.2:1.06+Math.pow(s(),2.6)*.34;xe[p*3]=j.x*se,xe[p*3+1]=j.y*se,xe[p*3+2]=j.z*se,je[p]=s()<.035?1.5+s()*1.4:.45+s()*.7,Ve[p]=s()}const ae=new Xe;ae.setAttribute("position",new x(xe,3)),ae.setAttribute("aSize",new x(je,1)),ae.setAttribute("aSeed",new x(Ve,1)),ae.boundingSphere=new De(new L,4),T=new Je(ae,qe),C.add(T),e.count=d,e.curves=a}const Se={type:ft,minFilter:Ye,magFilter:Ye,depthBuffer:!1};let P=null,F=[];const $=new We,de=new dt(-1,1,1,-1,0,1),he=new ht(new vt(2,2));$.add(he);const Ae="varying vec2 vUv; void main(){ vUv = uv; gl_Position = vec4(position.xy, 0.0, 1.0); }",Pe=new ie({uniforms:{tex:{value:null}},vertexShader:Ae,fragmentShader:"uniform sampler2D tex; varying vec2 vUv; void main(){ gl_FragColor = texture2D(tex, vUv); }"}),_=new ie({uniforms:{tex:{value:null},dir:{value:new we(1,0)},texel:{value:new we}},vertexShader:Ae,fragmentShader:`uniform sampler2D tex; uniform vec2 dir, texel; varying vec2 vUv;
    ...
    }`});function ee(){const t=g.clientWidth,a=g.clientHeight;if(!t||!a)return;i.setSize(t,a),H.aspect=t/a,H.updateProjectionMatrix(),A.uniforms.uAspect.value=t/a,l.uPx.value=a*R*.0105;const r=Math.round(t*R),u=Math.round(a*R);P&&P.dispose(),F.forEach(n=>{n.a.dispose(),n.b.dispose()}),P=new Te(r,u,Se),F=[2,4,8,16].map(n=>{const c=Math.max(2,Math.round(r/n)),b=Math.max(2,Math.round(u/n));return{a:new Te(c,b,Se),b:new Te(c,b,Se),w:c,h:b}})}window.addEventListener("resize",ee);const ve=window.ResizeObserver?new ResizeObserver(()=>ee()):null;ve&&ve.observe(g);function Qe(){i.setRenderTarget(P),i.clear(),i.render(Fe,H);let t=P.texture;for(const a of F)he.material=Pe,Pe.uniforms.tex.value=t,i.setRenderTarget(a.a),i.render($,de),he.material=_,_.uniforms.texel.value.set(1/a.w,1/a.h),_.uniforms.dir.value.set(1,0),_.uniforms.tex.value=a.a.texture,i.setRenderTarget(a.b),i.render($,de),_.uniforms.dir.value.set(0,1),_.uniforms.tex.value=a.b.texture,i.setRenderTarget(a.a),i.render($,de),t=a.a.texture;he.material=A,A.uniforms.tScene.value=P.texture,A.uniforms.tB0.value=F[0].a.texture,A.uniforms.tB1.value=F[1].a.texture,A.uniforms.tB2.value=F[2].a.texture,A.uniforms.tB3.value=F[3].a.texture,i.setRenderTarget(null),i.render($,de)}const Be=new mt,Ue=new we,Ze=new De(new L,1),Ee=new L,M=i.domElement;let te=null,_e=0;M.addEventListener("pointerdown",t=>{te={x:t.clientX,y:t.clientY},M.style.cursor="grabbing"}),M.addEventListener("pointerup",t=>{te&&Math.hypot(t.clientX-te.x,t.clientY-te.y)<6&&$e(),te=null,M.style.cursor="grab"}),M.addEventListener("pointermove",t=>{const a=M.getBoundingClientRect();Ue.set((t.clientX-a.left)/a.width*2-1,-((t.clientY-a.top)/a.height)*2+1),Be.setFromCamera(Ue,H),Be.ray.intersectSphere(Ze,Ee)?(C.worldToLocal(Ee),l.uPointer.value.copy(Ee),l.uPointerOn.value=1,_e=performance.now()):l.uPointerOn.value=0}),M.addEventListener("pointerleave",()=>{l.uPointerOn.value=0});function $e(){f.push({r:0,life:1}),e.pulse=Math.min(1.7,e.pulse+1),e.charge+=.2,e.charge>=1&&e.level<6?(e.charge=0,e.level++,Me(e.level),f.push({r:0,life:1.4}),e.pulse=1.7):e.level>=6&&(e.charge=1)}function et(t){e.mode=t,e.pulse=Math.min(1.7,e.pulse+.55),f.push({r:0,life:.85})}const tt={last:performance.now(),dt(){const t=performance.now(),a=(t-this.last)/1e3;return this.last=t,a}};let I=0;const Le=(t,a,r)=>t+(a-t)*r,h={deep:[0,0,0],mid:[0,0,0],hot:[0,0,0]};h.deep=U[0].deep.slice(),h.mid=U[0].mid.slice(),h.hot=U[0].hot.slice();function Ie(){if(!B)return;requestAnimationFrame(Ie);const t=Math.round(g.clientWidth*R),a=Math.round(g.clientHeight*R);if((M.width!==t||M.height!==a)&&ee(),!P)return;const r=(gt?.16:1)*Math.min(.05,tt.dt());I+=r;const u=Q[e.mode];e.micLevel*=.9;let n;e.force!==null?n=e.force:e.mode==="parle"?n=Math.max(0,.34+.3*Math.sin(I*7.3)+.22*Math.sin(I*11.9+1.7)+.14*Math.sin(I*19.1)):n=0;const c=e.force!==null||e.mode==="parle",b=Math.max(e.micLevel,Math.min(1,n));l.uVoice.value=n,l.uPitch.value=0;const E=Math.min(1,r*4),O=Math.min(1,r*13);e.energy+=(u.energy+b*.55+e.pulse*.25-e.energy)*(c?O:E),e.flow+=(u.flow+b*1.9+e.pulse*.8-e.flow)*(c?O:E),e.turb+=(u.turb+b*.75+e.pulse*.45-e.turb)*(c?O:E),c&&(y.autoRotateSpeed=.2+n*.9),e.pulse*=Math.pow(.15,r),e.charge=Math.max(0,e.charge-r*.02),(e.mode==="analyse"||e.mode==="surch"||e.mode==="pense")&&(e.charge=Math.min(1,e.charge+r*.035)),e.charge>=1&&e.level<6&&(e.charge=0,e.level++,Me(e.level),f.push({r:0,life:1.3})),performance.now()-_e>450&&(l.uPointerOn.value*=Math.pow(.05,r)),e.phase+=r*e.flow;const X=e.mode==="ecoute"||e.micLevel>.03?-1:1;e.dir+=(X-e.dir)*Math.min(1,r*2.2),e.iph+=r*(.22+.15*e.flow)*e.dir,l.uPhase.value=e.phase,l.uIph.value=e.iph,l.uDirS.value=e.dir>=0?1:-1;const q=e.teinte&&ue[e.teinte]||U[e.pal],k=Math.min(1,r*3);for(let o=0;o<3;o++)h.deep[o]=Le(h.deep[o],q.deep[o],k),h.mid[o]=Le(h.mid[o],q.mid[o],k),h.hot[o]=Le(h.hot[o],q.hot[o],k);l.uDeep.value.setRGB(h.deep[0],h.deep[1],h.deep[2]),l.uMid.value.setRGB(h.mid[0],h.mid[1],h.mid[2]),l.uHot.value.setRGB(h.hot[0],h.hot[1],h.hot[2]),A.uniforms.uExp.value=1.34-e.energy*.16,A.uniforms.uBloom.value=.9-e.energy*.34,l.uTime.value=I,l.uEnergy.value=e.energy,l.uFlow.value=e.flow,l.uTurb.value=e.turb,l.uPulse.value=e.pulse,l.uMic.value=b;for(let o=f.length-1;o>=0;o--){const v=f[o];v.r+=1.9*r,v.life-=r*.8,(v.life<=0||v.r>2.6)&&f.splice(o,1)}for(let o=0;o<4;o++){const v=f[o];l.uWaves.value[o].set(v?v.r:-9,v?Math.max(0,v.life):0,0,0)}y.autoRotateSpeed=(e.mode==="repos"?.08:.2)+u.spin*1.2,C.rotation.x=Math.sin(I*.1)*.1,C.rotation.z=Math.cos(I*.07)*.07,y.update(r),Qe()}ee(),Me(1),Ie();const Ne={idle:"repos",listening:"ecoute",thinking:"pense",speaking:"parle"},at=Object.fromEntries(Object.entries(Ne).map(([t,a])=>[a,t]));return{setState(t){const a=Ne[t]||(Q[t]?t:null);a&&a!==e.mode&&et(a)},setLevel(t){e.force=Math.max(0,Math.min(1,+t||0))},releaseLevel(){e.force=null},setTeinte(t){e.teinte=Object.prototype.hasOwnProperty.call(ue,t)?t:null},pulse(){f.push({r:0,life:1}),e.pulse=Math.min(1.7,e.pulse+1)},get state(){return at[e.mode]||e.mode},get teinte(){return e.teinte||"base"},dispose(){if(B){B=!1,window.removeEventListener("resize",ee),ve&&ve.disconnect();try{D&&D.geometry.dispose(),T&&T.geometry.dispose(),Oe.dispose(),qe.dispose(),Pe.dispose(),_.dispose(),A.dispose(),G.dispose(),P&&P.dispose(),F.forEach(t=>{t.a.dispose(),t.b.dispose()}),i.dispose(),M.parentNode&&M.parentNode.removeChild(M)}catch{}}}}}function St({etat:oe="idle",niveau:B=null,onToucher:le=null,taille:Q=240,teinte:U="base",remplir:ue=!1}){const e=ue?{width:"100%",height:"100%",display:"block",position:"relative"}:{width:Q,height:Q,margin:"0 auto",display:"block",position:"relative"},f=K.useRef(null),g=K.useRef(null);return K.useEffect(()=>{if(!f.current)return;let i=null;try{i=xt(f.current)}catch(R){console.warn("Loggia : orbe indisponible",R);return}return g.current=i,()=>{g.current=null,i.dispose()}},[]),K.useEffect(()=>{g.current&&g.current.setState(oe)},[oe]),K.useEffect(()=>{g.current&&g.current.setTeinte(U)},[U]),K.useEffect(()=>{const i=g.current;i&&(B==null?i.releaseLevel():i.setLevel(B))},[B]),le?Ce.jsx("button",{type:"button",onClick:le,"aria-label":st("Parler à l’assistant"),style:{...e,border:"none",padding:0,background:"transparent",cursor:"pointer"},children:Ce.jsx("span",{ref:f,style:{display:"block",width:"100%",height:"100%"}})}):Ce.jsx("div",{ref:f,style:e})}export{xt as creerOrbe,St as default};
