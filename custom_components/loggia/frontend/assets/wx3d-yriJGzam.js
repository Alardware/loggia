import{j as A,W as q}from"./index-BS0tZE76.js";import{W as D,S as z,O as H,P as V,a as j,V as h,M as G,C as O}from"./three-mUwG1-rm.js";import{r as p}from"./vendor-nf7bT_Uh.js";const _=`
...
float rainLayer(vec2 uv, float scale, float speed, float slant, float len, float seed, float amt){
  ...
  float col = floor(g.x);
  float sp = speed * (0.80 + 0.5 * hash21(vec2(col, seed))) * (0.65 + 1.15 * amt);
  ...
  float w = (0.055 + 0.070 * y) * (0.72 + 0.55 * amt);
  float body = smoothstep(w, 0.0, d);
  float lenA = len * (0.55 + 1.30 * amt);
  float along = smoothstep(lenA, 0.0, y) * smoothstep(0.0, 0.012, y);
  return body * pow(along, mix(1.5, 0.85, amt));
}
...
void main(){
  ...
  if (uRain > 0.001) {
    float r = 0.0;
    r += rainLayer(su, 76.0, 1.35, slant, 0.44, 3.0, uRain) * 0.55;
    r += rainLayer(su, 46.0,  1.05, slant, 0.38, 9.0, uRain) * 0.85;
    r += rainLayer(su, 27.0,  0.78, slant, 0.32, 17.0, uRain) * 1.1;
    vec3 rc = mix(vec3(0.90, 0.95, 1.02), vec3(0.28, 0.34, 0.48), nightW);
    col += rc * r * uIntensity * (0.55 + 0.62 * uRain);
    // a torrential downpour reads as a moving veil, not just more drops
    float veil = smoothstep(0.55, 1.0, uRain);
    if (veil > 0.001) {
      vec2 vu = su * vec2(2.2, 0.55) + vec2(uTime * 0.05 * slant, uTime * 0.075);
      col += rc * veil * (0.022 + 0.050 * fbm(vu * 3.0)) * uIntensity;
    }
  }
  if (uHail > 0.001) {
    float g = 0.0;
    g += rainLayer(su, 62.0, 2.4, slant * 0.5, 0.14, 31.0, uHail);
    g += rainLayer(su, 40.0, 1.9, slant * 0.5, 0.12, 43.0, uHail);
...
}`;function Y({condition:x="partlycloudy",hourEq:y=12,intensity:g=1}){const w=p.useRef(null),n=p.useRef({condition:x,hourEq:y,intensity:g});return n.current={condition:x,hourEq:y,intensity:g},p.useEffect(()=>{const l=w.current;if(!l)return;let C=!1,W=0,a;try{a=new D({antialias:!1})}catch{return}a.setPixelRatio(Math.max(1,Math.min(window.devicePixelRatio||1,1.5)));const m=a.domElement;m.style.cssText="position:absolute;inset:0;display:block;width:100%;height:100%;",l.appendChild(m);const k=new z,L=new H(-1,1,1,-1,0,1),e={uRes:{value:new h(1,1)},uTime:{value:0},uElev:{value:.6},uSunPos:{value:new h(.5,.8)},uMoonPos:{value:new h(.5,-.4)},uCloud:{value:0},uDark:{value:0},uFog:{value:0},uRain:{value:0},uSnow:{value:0},uHail:{value:0},uWind:{value:0},uStars:{value:0},uFlash:{value:0},uBolt:{value:0},uBoltSeed:{value:0},uExc:{value:0},uIntensity:{value:1}},b=new V(2,2),S=new j({uniforms:e,vertexShader:"varying vec2 vUv; void main(){ vUv = uv; gl_Position = vec4(position.xy, 0.0, 1.0); }",fragmentShader:_});k.add(new G(b,S));const M=()=>{const o=l.clientWidth||1,i=l.clientHeight||1;a.setSize(o,i,!1),e.uRes.value.set(o*a.getPixelRatio(),i*a.getPixelRatio())},R=new ResizeObserver(M);R.observe(l),M();const T=["cloud","dark","fog","rain","snow","hail","wind","stars","exc"],r={};T.forEach(o=>{r[o]=0});let u=.2;const E=new O;let s=0,v=0,d=2;function P(){if(C||(W=requestAnimationFrame(P),document.hidden))return;const o=Math.min(E.getDelta(),.05),i=E.elapsedTime;e.uTime.value=i;const f=q[n.current.condition]||q.partlycloudy;let c=n.current.hourEq;f.forceNight&&(c=23.2),f.forceDay&&(c=Math.min(Math.max(c,8),17));const I=(c-6)/12*Math.PI,U=Math.sin(I);u+=(U-u)*Math.min(1,o*2.2),e.uElev.value=u;const B=.68+.26*Math.sin((c-6)/12*Math.PI-Math.PI/2)*-1;e.uSunPos.value.set(B,.18+u*.72),e.uMoonPos.value.set(1-B,.18-u*.72),T.forEach(t=>{r[t]+=((f[t]||0)-r[t])*Math.min(1,o*2.4),e["u"+t[0].toUpperCase()+t.slice(1)].value=r[t]}),e.uIntensity.value=n.current.intensity;const F=f.strike||0;if(F>0){d-=o*F,d<=0&&(d=1.6+Math.random()*4.5,s=1,v=Math.random()<.75?1:0,e.uBoltSeed.value=Math.random()*100);const t=.55+.45*Math.sin(i*90);e.uFlash.value=s*t*.85,e.uBolt.value=v*Math.pow(s,.4)*t,s=Math.max(0,s-o*3.4),s<.02&&(v=0)}else s=0,v=0,e.uFlash.value+=(0-e.uFlash.value)*Math.min(1,o*6),e.uBolt.value=0;a.render(k,L)}return P(),()=>{C=!0,cancelAnimationFrame(W),R.disconnect(),b.dispose(),S.dispose(),a.dispose();try{l.removeChild(m)}catch{}}},[]),A.jsx("div",{ref:w,"aria-hidden":"true",style:{position:"absolute",inset:0}})}export{q as WX_PRESETS,Y as default};
