...
test('la feuille qui porte l’orbe est opaque', () => {
  ...
  assert.match(UI, /export function BottomSheet\(\{ onClose, children, opaque = false, onglets = false, title = null \}\)/);
  assert.match(UI, /background: opaque \? 'linear-gradient\(var\(--o-surfA\), var\(--o-surfA\)\), var\(--o-bg\)' : 'var\(--o-surfA\)'/);
  assert.match(UI, /className=\{'o-sheet' \+ \(opaque \? ' o-sheet-opaque' : ''\) \+ \(onglets \? ' o-sheet-onglets' : ''\)\}/);
