...
test('les sections : Securite en tete de la colonne, En ce moment en tete du rail, et les anciens noms migrent', () => {
  ...
  assert.ok(o.includes("return ['securite', ...sauve, ...manquants.filter(s => s !== 'securite')];"), 'Securite passe en tete d’un accueil deja range');
...
test('la carte Securite : les boutons d’armement d’aujourd’hui, la serrure, les ouvrants — ou rien', () => {
  ...
  assert.ok(home.includes("tr('{n} ouvrants ouverts sur {m}', { n: ouvStat.ouverts, m: ouvStat.total })") && home.includes("tr('Tout est fermé')") && home.includes("'securite') : null;"), 'les ouvrants, comptes, vers la vue Securite');
  assert.ok(home.includes('const carteSecurite = (alarmRailId || serrureId || ouvrantsRow) ? ('), 'sans panneau, serrure ni ouvrant : pas de carte');
