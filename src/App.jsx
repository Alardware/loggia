...
const NAV = [
  { group: 'MAISON', items: [
    ...
    { label: 'Pièces', svg: <Fi i="door-open" color="var(--o-orange)" /> },
...
const LABEL_VIEW = { 'Accueil': 'accueil', 'Pièces': 'pieces', 'Lumières': 'lumieres', 'Scénarios': 'scenes', 'Climat': 'climat', 'Volets': 'volets', 'Énergie': 'energie', 'Croquettes': 'croquettes', 'Médias': 'medias', 'Objets': 'objets', 'Sécurité': 'securite', 'Caméras': 'cameras', 'Système': 'systeme', 'Paramètres': 'parametres' };
...
function SearchSheet({ onClose, onNav, customViews = [], rooms = [], droits = [] }) {
  ...
  if (nq.length >= 2) {
    ...
    // Un robot n'a pas de vue : sa carte, dans Objets, ouvre sa fiche.
    const VUE_DOM = { climate: 'climat', cover: 'volets', media_player: 'medias', vacuum: 'objets', lawn_mower: 'objets' };
    let n = 0;
    for (const id in S) {
      if (n >= 8) break;
      ...
      <div className="o-piece o-piecechip" onClick={onOpen} role="button" tabIndex={0}
        ...
        {canToggle && (
          <span role="switch" aria-checked={on} aria-label={tr('Lumières') + ' ' + p.name} tabIndex={0}
            ...
            style={{ width: 44, height: 25, borderRadius: 14, position: 'relative', cursor: 'pointer', flexShrink: 0, background: on ? 'linear-gradient(135deg,var(--o-lampe),var(--o-lampe-b))' : 'var(--o-bd1)', transition: 'background .25s' }}>
...
function outdoorTips(mode, temp, wind, isNight, rainProb) {
  ...
  if (temp != null) {
    ...
    else if (temp >= 30) T.push(['humidity', 'var(--o-orange)', 'Forte chaleur, pense à t’hydrater']);
    else if (temp >= 25) T.push(['sun', 'var(--o-lampe)', 'Il fait chaud, vêtements légers conseillés']);
  }
...
function RoomFeederCard({ nom, sub, pct, prochaine, onFeed, onRempli = null, onOpen, extra = null, chip = false, mort = false }) {
    ...
    <BottomSheet onClose={onClose}>
      {() => (<>
        ...
        {(zone.tempSensor || estClimate(zone)) && (<>
          ...
          <Courbe24 points={ptsTemp} couleur="var(--o-orange)" unite="°" />
        </>)}
      </>)}
    </BottomSheet>
  );
...
function NavigateurMedias({ id, hass, onClose }) {
  ...
  const ouvrir = (c) => {
    ...
    /* Pas de `try/catch` : il n'attraperait rien. `callService` rend une
     * promesse, et un refus la rejette plus tard, hors de portée d'un bloc
     * synchrone. Le rejet remonte à l'écoute globale, qui affiche « Commande
     * non exécutée » — c'est bien le lecteur qui dit s'il a refusé, mais par ce
     * chemin-là, pas par un `catch` qui ne voit jamais rien passer. */
...
const DOMAINES_EDITION = () => [
  ...
  { id: 'multimedia', label: tr('Multimédia'), fi: 'tv-music', rgb: 'var(--o-rose-rgb)' },
...
function FicheMachineHero({ id, hass }) {
  ...
  return (
    <div className={mort ? 'o-panne' : undefined} style={{ ...CV_CADRE, opacity: mort ? .55 : 1 }}>
...
/* La fiche d'un robot — aspirateur ou tondeuse — dans une feuille, comme les
 * autres appareils (ajustement de l'ADR 0042, 18/09) : le meme rendu sur PC,
 * tablette et telephone. Le contenu (onglets, planning, entretien, reglages)
 * est charge a la demande, DANS une frontiere Suspense — sans elle, un clic
 * finissait dans l'ecran d'erreur. La fiche vit a son rythme : le poll de la
 * vue derriere ne connait pas les entites soeurs du robot, et sans cet
 * abonnement une jauge ou une bascule resterait figee. */
function FicheRobot({ id, hass, onClose }) {
  const domaine = String(id).split('.')[0];
  const hassLive = useHass([id, ...siblingsOf(LOGGIA_INDEX, id)]);
  const H = hassLive || hass;
  // La fiche universelle (Stop, Localiser, toutes les entites) s'ouvre
...
function useDomainCards(hass, { onNav = null } = {}) {
  ...
  const [robotPop, setRobotPop] = useState(null);
  ...
  const ouvrir = (id) => {
    ...
    // Un robot a sa fiche a lui (ADR 0042) : accueil, zones, planning,
    // historique, entretien, reglages — une feuille, comme les autres appareils.
    else if (DOMAINES_ROBOT.indexOf(d) >= 0) setRobotPop(id);
    else setAppPop(id);
  };
  ...
  const fermer = () => { setLightPop(null); setClimPop(null); setPilotPop(null); setCoverPop(null); setMediaPop(null); setSensPop(null); setAppPop(null); setCamPop(null); setPrisePop(null); setLockPop(null); setBinPop(null); setRobotPop(null); setFeederPop(false); setPlantPop(null); };
...
const CAMERAS = () => [
  { cle: 'demo-entree', label: tr('Entrée'), tag: 'LIVE · ENTRÉE', grad: 'linear-gradient(180deg,#6ba8d8 0%,#9cc4e0 42%,#7a8a5c 60%,#56683f 100%)', glow: 'radial-gradient(120% 80% at 50% 18%,rgba(255,255,255,.18),transparent 55%)', sub: <><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--o-lampe)" strokeWidth="2.4" strokeLinecap="round"><path d="M13 2L3 14h7l-1 8 11-13h-7z" /></svg>Mouvement il y a 3 min</> },
...
function sousCamera(online, ev) {
  ...
  return <><Fi i={ev.icone} size={11} color="var(--o-lampe)" />{ev.libelle + ' · ' + depuis(ev.quand)}</>;
}
...
function verdictCartePlante(pl) {
  ...
  const rgb = v.presse.length ? 'var(--o-orange-rgb)' : v.hum === 'humide' ? '34,211,238' : 'var(--o-ok-rgb)';
 ...
 * sur une vue metier.
 */
function ViewHead({ titre, sous, badge, rgb = 'var(--o-ok-rgb)' }) {
  return (
...
const TEINTES_PIECE = [
  { id: 'accent', label: 'Accent', col: 'var(--o-accent-soft)', rgb: 'var(--o-accent-soft-rgb)' },
                  ...
                  {avatars.map((u, i) => {
                    ...
                    return (
                      <span key={i + (present ? '-p' : '-a')} className="o-avatarin" title={u.title} style={{ position: 'relative', width: 34, height: 34, flexShrink: 0, display: 'inline-block' }}>
                        <span style={{ display: 'block', width: '100%', height: '100%', borderRadius: '50%', background: u.img ? `url(${u.img}) center/cover` : u.grad, boxShadow: present ? '0 0 0 2.5px var(--o-ok), 0 0 9px rgba(var(--o-ok-rgb),.5)' : '0 0 0 2px var(--o-bd1)', opacity: present ? 1 : 0.45 }} />
                        <span style={{ position: 'absolute', right: -1, bottom: -1, width: 12, height: 12, borderRadius: '50%', background: present ? 'var(--o-ok)' : 'var(--o-text3)', border: '2.5px solid var(--o-bg2)', boxShadow: present ? '0 0 6px rgba(var(--o-ok-rgb),.7)' : 'none' }} />
                      </span>
                    );
                  })}
                </div>
...
const hx = (hex, a) => { if (HX_TOKENS[hex]) return `rgba(var(${HX_TOKENS[hex]}),${a})`; if (typeof hex !== 'string' || hex[0] !== '#') return `rgba(var(--o-text3-rgb),${a})`; const n = parseInt(hex.slice(1), 16); return `rgba(${(n >> 16) & 255},${(n >> 8) & 255},${n & 255},${a})`; };
...
const LIGHT_PALETTE = ['var(--o-lampe)', 'var(--o-orange)', '#f472b6', 'var(--o-purple)', 'var(--o-accent)', 'var(--o-cyan)', 'var(--o-ok)', '#ffffff'];
/* Une FONCTION, pas une table.
 *
 * Evaluee a l'import, cette liste figeait ses libelles dans la langue du
 * demarrage. C'est ce qui obligeait a recharger la page apres un changement de
...
function EnergyHouseSchema({ solarW = 47, homeW = 907, surplusW = 954, evW = 0, evBranche = false, batW = 0, batSoc = null, batPresente = false }) {
  ...
  const C = { solar: 'var(--o-flux-solaire)', home: 'var(--o-flux-maison)', grid: 'var(--o-flux-reseau)', ev: 'var(--o-flux-ve)', bat: 'var(--o-flux-batterie)' };
...
const EN_LOOK = [['briefcase', 'var(--o-cyan)'], ['microchip', 'var(--o-purple)'], ['thermometer-half', 'var(--o-orange)'], ['utensils', 'var(--o-accent-soft)'], ['bolt', 'var(--o-rose)']]; // 'plug' n'existe pas dans la fonte UICons
...
function SecuriteContent({ hass, edit = false, onEnt, onNav = null }) {
  ...
  const statusCol = unknownState ? 'var(--o-text3-rgb)' : triggered ? 'var(--o-bad-rgb)' : alarm !== 'off' ? 'var(--o-warn-rgb)' : 'var(--o-ok-rgb)';

  const cs = a => `rgb(${a})`;
  const ca = (a, al) => `rgba(${a},${al})`;
...
function cvTypesPour(id) {
  const d = String(id).split('.')[0];
  // La CHIP ferme chaque liste : c'est la plus petite carte du catalogue, une
  // pastille pour une entité (retour 01/09).
  ...
  if (d === 'light' || d === 'switch' || d === 'fan') return ['compacte', 'riche', 'journal', 'chip'];
  if (d === 'cover' || d === 'climate' || d === 'media_player' || d === 'vacuum' || d === 'lawn_mower' || d === 'valve' || d === 'humidifier' || d === 'lock' || d === 'siren' || d === 'water_heater') return ['compacte', 'riche', 'journal', 'chip'];
  if (d === 'scene' || d === 'script' || d === 'button' || d === 'input_button' || d === 'automation') return ['compacte', 'riche', 'chip'];
  // La jauge a quitté le catalogue le 31/08/2026 (« il est moche ») — les
  // cartes déjà posées continuent de se rendre (CvTyped).
  if (d === 'sensor') return ['compacte', 'chiffre', 'graph', 'journal', 'chip'];
  if (d === 'binary_sensor') return ['compacte', 'chiffre', 'journal', 'chip'];
  if (d === 'person') return ['compacte', 'personne', 'journal', 'chip'];
  if (d === 'weather') return ['compacte', 'meteo', 'chip'];
  if (d === 'calendar') return ['agenda', 'calendrier', 'compacte', 'chip'];
  if (d === 'alarm_control_panel') return ['compacte', 'alarme', 'alarmeseule', 'riche', 'chip'];
...
/* Valeur COURTE d'une entité pour une pastille : ce qui tient à côté de son
 * icône. Une porte n'a pas besoin de mot — son icône et sa couleur le disent
 * déjà ; un capteur, si. */
...
function CvChip({ id, hass, dc = null }) {
  ...
  const col = d === 'cover' ? 'var(--o-purple)' : d === 'climate' ? 'var(--o-bad)'
    : (d === 'light' || d === 'switch') ? 'var(--o-warn)'
      : d === 'alarm_control_panel' ? (st && st.state === 'triggered' ? 'var(--o-bad)' : st && st.state === 'disarmed' ? 'var(--o-ok)' : 'var(--o-warn2)')
        : 'var(--o-accent-soft)';
  const val = chipTexte(id, st);
  const nom = cvName(st, id);
  return (
...
function CvChips({ x = null, hass, dc = null, demo = null }) {
  ...
  const chips = useMemo(() => {
    ...
    return ids.map(id => {
      ...
      const col = d === 'cover' ? 'var(--o-purple)' : d === 'climate' ? 'var(--o-bad)'
        : (d === 'light' || d === 'switch') ? 'var(--o-warn)'
          : d === 'alarm_control_panel' ? (st.state === 'triggered' ? 'var(--o-bad)' : st.state === 'disarmed' ? 'var(--o-ok)' : 'var(--o-warn2)')
            : 'var(--o-accent-soft)';
  ...
  return (
    <div className="o-piece" style={{ ...CV_CADRE, height: '100%', minHeight: 172, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        ...
        {part != null && <span style={{ fontSize: 11, fontWeight: 800, padding: '3px 9px', borderRadius: 999, background: 'rgba(var(--o-ok-rgb),.14)', color: 'var(--o-ok)' }}>{tr('Solaire')} {part} %</span>}
      </div>
      <div style={{ marginTop: 8 }}>
...
function BiblioView() {
  ...
  return (
    <main className="loggia-main" style={{ flex: 1, minWidth: 0, padding: '26px 28px 40px', overflowX: 'hidden' }}>
      ...
      <Titre i="thermometer-half" c="var(--o-orange)" t={tr('Climat')} />
      <Rangee>
      ...
      <Rangee>
        ...
        <Item l={tr('Chip (capteur)')} h={88}><CvChip id="sensor.biblio_co2" hass={hb} dc={dc} /></Item>
...
function ChipsEditSheet({ x, hass, onClose, onSave }) {
  ...
  const poser = (l) => { setIds(l); onSave({ ...x, ids: l.length ? l : undefined }); };
  ...
  return (
    <BottomSheet onClose={onClose}>
      {() => (<>
        ...
        <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--o-text3)', letterSpacing: '.04em', marginBottom: 8 }}>{tr('AJOUTER UNE PASTILLE')}</div>
        <EntPicker hass={hass} exclude={ids} onPick={(id) => poser([...ids, id])} autoFocus />
      </>)}
    </BottomSheet>
  );
}
...
function CustomView({ cv, hass, edit = false, onSave }) {
  ...
  // Carte posée dont on change le dessin (crayon), et carte chips dont on
  // compose la rangée.
  const [retype, setRetype] = useState(null);
  const [chipsEdit, setChipsEdit] = useState(null);
  ...
  return (
    ...
    <div className={navbar ? 'o-navbar-on' : undefined} style={{ display: 'flex', minHeight: '100vh', background: fondPhotoActif ? 'transparent' : 'var(--o-bggrad, var(--o-bg))', fontFamily: 'var(--o-font)', color: 'var(--o-text)',
      ...
      <Sidebar view={view} vuesAutorisees={vuesAutorisees} editMode={editMode} onToggleEdit={peutEditer ? () => setEditMode(e => !e) : null} onNav={(v) => { setView(v); try { if ((window.innerWidth || 0) <= 820) setNavOpen(false); } catch {} }} open={navOpen} customViews={customViews} ha={(() => {
        ...
        const al = ast == null ? { t: tr('Alarme · état inconnu'), c: 'var(--o-text3-rgb)' }
