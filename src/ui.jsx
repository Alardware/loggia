...
export const HIDDEN_VIEWS = [
  { label: 'Lumières', vid: 'lumieres', icon: 'bulb', c: '#ffce73' },
  { label: 'Climat', vid: 'climat', icon: 'thermometer-half', c: '#ff8a4c' },
  { label: 'Volets', vid: 'volets', icon: 'blinds', c: 'var(--o-accent-soft)' },
  { label: 'Aspirateur', vid: 'aspirateur', icon: 'broom', c: 'var(--o-accent-soft)' },
  { label: 'Croquettes', vid: 'croquettes', icon: 'paw', c: '#ffce73' },
  { label: 'Médias', vid: 'medias', icon: 'tv-music', c: 'var(--o-purple)' },
  { label: 'Météo', vid: 'meteo', icon: 'cloud-sun', c: 'var(--o-cyan)' },
];
...
export function EntPicker({ hass, exclude = [], onPick, autoFocus = false }) {
  ...
  return (
    <>
      <input value={q} onChange={e => setQ(e.target.value)} placeholder="Rechercher une entité (nom ou id)…" spellCheck={false} autoFocus={autoFocus} style={cvInp} />
      {results.length > 0 && (
