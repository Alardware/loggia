// Fond météo GLSL plein cadre (handoff « design_handoff_weather_effects » v2, 20/08/2026).
// RÈGLE DU HANDOFF : le shader FRAG et la table WX_PRESETS sont la SPÉCIFICATION, copiés
// verbatim — ne pas les réécrire. Seul l'hôte (montage, cycle de vie, plomberie d'état)
// est adapté à Loggia : conteneur au lieu de body, pause onglet caché, dispose au démontage.
...
// hôte React : monte le rendu dans son conteneur ; props pilotées par ref (pas de re-création)
export default function WeatherGL({ condition = 'partlycloudy', hourEq = 12, intensity = 1 }) {
  const hostRef = useRef(null);
  const propsRef = useRef({ condition, hourEq, intensity });
  propsRef.current = { condition, hourEq, intensity };
  useEffect(() => {
    ...
    const U = {
      ...
      uMoonPos: { value: new THREE.Vector2(0.5, -0.4) },
      uCloud: { value: 0 }, uDark: { value: 0 }, uFog: { value: 0 },
      uRain: { value: 0 }, uSnow: { value: 0 }, uHail: { value: 0 },
      uWind: { value: 0 }, uStars: { value: 0 },
      uFlash: { value: 0 }, uBolt: { value: 0 }, uBoltSeed: { value: 0 },
      uExc: { value: 0 }, uIntensity: { value: 1 },
    };
    ...
    const KEYS = ['cloud', 'dark', 'fog', 'rain', 'snow', 'hail', 'wind', 'stars', 'exc'];
    const cur = {}; KEYS.forEach(k2 => { cur[k2] = 0; });
    let elevCur = 0.2;
    ...
    function frame() {
      ...
      if (document.hidden) return; // onglet caché : pas de rendu
      const dt = Math.min(clock.getDelta(), 0.05);
      const t = clock.elapsedTime;
      U.uTime.value = t;

      const p = WX_PRESETS[propsRef.current.condition] || WX_PRESETS.partlycloudy;
      // heure « solaire équivalente » fournie par Loggia (NOAA) ; forceDay/forceNight des presets respectés
      let hour = propsRef.current.hourEq;
      if (p.forceNight) hour = 23.2;
      if (p.forceDay) hour = Math.min(Math.max(hour, 8), 17);
      const a = ((hour - 6) / 12) * Math.PI;
      const targetElev = Math.sin(a);
      elevCur += (targetElev - elevCur) * Math.min(1, dt * 2.2);
      U.uElev.value = elevCur;
      const ax = 0.68 + 0.26 * Math.sin(((hour - 6) / 12) * Math.PI - Math.PI / 2) * -1;
      U.uSunPos.value.set(ax, 0.18 + elevCur * 0.72);
      U.uMoonPos.value.set(1 - ax, 0.18 - elevCur * 0.72);

      KEYS.forEach(k2 => {
        cur[k2] += ((p[k2] || 0) - cur[k2]) * Math.min(1, dt * 2.4); // crossfade ~1 s : partie du design
        U['u' + k2[0].toUpperCase() + k2.slice(1)].value = cur[k2];
      });
      U.uIntensity.value = propsRef.current.intensity;

      const strike = p.strike || 0;
      if (strike > 0) {
        nextStrike -= dt * strike;
        if (nextStrike <= 0) {
          nextStrike = 1.6 + Math.random() * 4.5;
          flash = 1;
          bolt = Math.random() < 0.75 ? 1 : 0;
          U.uBoltSeed.value = Math.random() * 100;
        }
        const flick = 0.55 + 0.45 * Math.sin(t * 90);
        U.uFlash.value = flash * flick * 0.85;
        U.uBolt.value = bolt * Math.pow(flash, 0.4) * flick;
        flash = Math.max(0, flash - dt * 3.4);
        if (flash < 0.02) bolt = 0;
      } else {
        flash = 0; bolt = 0;
        U.uFlash.value += (0 - U.uFlash.value) * Math.min(1, dt * 6);
        U.uBolt.value = 0;
      }
